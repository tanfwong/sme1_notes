...
\item An \textbf{field (algebra)} is a collection of subsets of
  \(\Omgea\) that is closed under union and complement, and contains
  both \(\emptyset\) and \(\Omega\). That is, \(\mathcal{F}\) is a
  field (algebra) if it satisfies the following three conditions:
\begin{enmerate}
\item[(i)] \(\emptyset \in \mathcal{F}\) and \(\Omega \in \mathcal{F}\).
\item[(ii)] If \(A \in \mathcal{F}\) and \(B \in \mathcal{F}\), then \(A \cup B \in \mathcal{F}\). 
\item[(iii)] If \(A \in \mathcal{F}\), then \(A^{c} \in \mathcal{F}\).
\end{itemize}
